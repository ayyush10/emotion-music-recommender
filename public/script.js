// <CHANGE> modernize camera/detection + safe DOM + better overlay placement + token handling
...
function startDetection() {
  ...
  detectIntervalId = setInterval(async () => {
    ...
    resized.forEach((d) => {
      ...
      const max = Math.max(...Object.values(exps), -Infinity);
...
async function getRecommendations(emotion) {
  try {
    // Replace this with a fresh OAuth token. Don’t hardcode in production.
    const token = (window.SPOTIFY_TOKEN || "BQChKVRyrna4pNlYHENhd3ARet6s7PvE6XF9I25GQ28EGmR7KoiYMcaPOVrDdm00E0o9P7deCjO55b_m9jFZIxusU3n912PY5FbL1OAdcTFn4eAvIFn6YcLtw9_74YYavrkX_vBsA1rauWMP5gbBoXPhmPP_3YU-mptj0SwKy9u-bqtn_1C4Yn2RLs9dWCCyGMfZh57TF2yGyaElUeIyL2mFNsq_NTxrJLaomAeulu88qslASKk0b-8OQL8NPCIoVQrxM1VnyGs70Nkh6VPFmvxb3LmHn6Kz6uYwel_OSbqF7Mf5XDg_k0nKNP_sRVc4").trim();
    if (!token) {
      console.warn("[v0] Missing Spotify token. Set window.SPOTIFY_TOKEN before calling getRecommendations().");
      return;
    }
